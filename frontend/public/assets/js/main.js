...
$(document).on(
    "keyup",
    'input[name="name"], input[name="firstname"], input[name="organisation"], input[name="issue_name"], input[name="milestones[]"], input[name="region_name"]',
    function(e) {
        name = $(this).val();
...
$(".menu-item-master_plan").find("a").attr("target", "_blank");
$(".sub-menu-item-edit_master_plan").find("a").attr("target", "");

$(document).on("click", ".modal-backdrop", function(e) {
    $("button.close, .assign-modal button.btn-default").trigger("click");
});

$(document).on("change", "select", function(e) {
    if ($(this).next("button").hasClass("bs-placeholder")) {
        $(this)
            .closest(".form-select-field")
            .find(".select-label")
            .removeClass("label-up-select");
    } else {
        $(this)
            .closest(".form-select-field")
            .find(".select-label")
            .addClass("label-up-select");
    }
});
$(document).on(
    "show.bs.modal",
    "#assignTicketPopup,#rejectTicketPopup",
    function() {
        $(".lightgallery").lightGallery();
    }
);
$(document).on("change", ".singleSelect select", function(e) {
    if ($(".singleSelect select option:selected").val() == "") {
        $(this)
            .closest(".singleSelect")
            .find(".select-label")
            .removeClass("label-up-select");
    }
});
...
$(document).click(function() {
    $(".btn-container").siblings().find("ul").slideUp("fast");
});

$(document).on("click", ".btn-container .btn", function(e) {
    $(this).closest(".btn-container").siblings().find("ul").slideUp("fast");
    $(this).closest(".dashboard-row").siblings().find("ul").slideUp("fast");
...
$(document).ready(function(e) {
    ...
    if ($(window).width() < 769) {
        ...
        $("#wrapper").css("margin-top", headHeight + 10 + "px");
    }

    if ($(window).width() < 991) {
        $(".dashboardModal").removeClass("mCustomScrollbar");
        $(".dashboardModal").mCustomScrollbar("destroy");
    }

    $("body").on("show.bs.modal", "#ticketDetailsPopup", function(e) {
        $(".dashboardModal").removeClass("mCustomScrollbar");
        $(".dashboardModal").mCustomScrollbar("destroy");
    });

    $(".modal").on("hidden.bs.modal", function(event) {
        if (this.className !== "modal fade sidebarModal dash-filter") {
            $(this)
                .find("form")
                .find("select")
                .val("default")
            ...
            $(this).find("form").find(".select-label").removeClass("label-up-select");

            $(".text-danger").css("display", "none");
            $(".floatingBtn").remove();

            if ($(".form-input-field input").val() !== "") {
...
$(document).on("click", "a[href='#ticket-history']", function(e) {
    $("body").append(
        '<button class="floatingBtn scroll-bottom btn btn-custom"><i class="fa fa-angle-double-down" aria-hidden="true"></i> Go to Latest</button>'
    );
});
$(document).on("click", "a[href='#ticket-details']", function(e) {
    $(".floatingBtn").remove();
});

$(document).on("click", ".floatingBtn", function() {
    let divHeight = $("#ticket-history").innerHeight();
    let ticketDataHeight = $(".ticket-data").innerHeight();
    let wHeight = window.innerHeight;
    console.log("Awnish-divHeight", ticketDataHeight, divHeight, wHeight);
    // $('#mCSB_2_container').css('top',-(height-400));
    // $('#mCSB_2_container').css('top',height);
    if ($(".floatingBtn").hasClass("scroll-bottom")) {
        $(".floatingBtn").html(
            '<i class="fa fa-angle-double-up" aria-hidden="true"></i> Go to Oldest'
        );
        $(".floatingBtn").addClass("scroll-top");
        $(".floatingBtn").removeClass("scroll-bottom");
        // $('#mCSB_2_container').animate({top : -(divHeight-400)}, 500);
        $(".mCustomScrollbar")
            .find(".mCSB_container, .mCSB_dragger")
            .animate({ top: wHeight - ticketDataHeight - 26 }, 500);
        // $(".mobile.modal-open .modal").animate(
        //   { top: wHeight - ticketDataHeight - 52 },
        //   500
        // );
        // $("#ticketDetailsPopup.modal").animate(
        //   { top: wHeight - ticketDataHeight },
        //   500
        // );

        $("#ticketDetailsPopup.modal").animate({ scrollTop: $('#ticketDetailsPopup.modal .modal-body').height() }, 1000);

        $(".mCustomScrollbar").find(".mCSB_dragger").animate({ top: 0 }, 500);

        //$('#mCSB_2_container').animate({bottom : 0}, 500);
    } else {
        $(".floatingBtn").html(
            '<i class="fa fa-angle-double-down" aria-hidden="true"></i> Go to Latest'
        );
        $(".floatingBtn").addClass("scroll-bottom");
        $(".floatingBtn").removeClass("scroll-top");
        // $('#mCSB_2_container').css('top',0);
        $(".mCustomScrollbar").find(".mCSB_container").animate({ top: 0 }, 500);
        $(".mCustomScrollbar").find(".mCSB_dragger").animate({ bottom: 0 }, 500);
        $(".mobile.modal-open .modal").animate({ top: 0 }, 500);
        $("#ticketDetailsPopup.modal").animate({ scrollTop: 0 }, 1000);
    }
});

$(document).on("click", ".show_milestone", function(e) {
    ...
    if ($(this).hasClass("plus")) {
        ...
        $(this).parents("tr").addClass("dt_bold");
        $("tr.milestone." + tr_id).show();
    } else {
        $(this).html('<i class="fa fa-plus-square-o"></i>');
        $(this).addClass("plus");
        $(this).parents("tr").removeClass("dt_bold");
        $("tr.milestone." + tr_id).hide();
    }
});
...
$(document).ajaxStop(function() {
    //$("button[type='submit']").waitMe("hide");
    $("#wrapper").waitMe("hide");
    $(".modal").waitMe("hide");
});
...
$(document).ajaxStart(function() {
    ...
    add_loader($container);
});

function add_loader($container) {
    $($container).waitMe({
        effect: "bounce",
        text: "",
        color: "#000",
...
$(window).on("load", function() {
    ...
    $(".header-search.show-search input").blur(function() {
        $(".header-search.show-search").removeClass("show-search");
    });
...
$(function() {
    ...
    $(document).on("app.form-validate", function(e, form) {
        ...
        $(form).on("change", "select.ajax-search, select.selectpicker", function(
            e
        ) {
            if ($(this).selectpicker("val") && !$(this).is(":disabled")) {
                if (
                    ...
                    parent.find("#" + $(this).attr("name") + "-error").hide();
    ...
    $("body").on("keypress", 'textarea[name="checklist-description"]', function(
        event
    ) {
        if (event.which == "13") {
            var that = $(this);
    ...
    $("body").on("show.bs.modal", "#lead-modal", function(e) {
        if ($("#lead-more-dropdown").find("li").length == 0) {
            $("#lead-more-btn").css("opacity", 0).css("pointer-events", "none");
        }
    });
    ...
    if (table_leads.length) {
        ...
        _table_api = initDataTable(
            ...
            LeadsServerParams, [table_leads.find("th.date-created").index(), "desc"]
        );
    ...
    $("form")
        .not("#single-ticket-form,#calendar-event-form,#proposal-form")
        .areYouSure();
    ...
    $("body").on("change loaded.bs.select", "#item_select", function() {
        ...
        if (selectAddon.length === 0) {
            ...
            $(".items-wrapper .bootstrap-select").css(
                "width",
                selectWrapper.width() - selectAddon.width() + 12 + "px"
            );
        }
    });
    ...
    $("body").on(
        "click" + (navigator.userAgent.match(/iPad/i) != null ? " touchstart" : ""),
    ...
    if (table_activity_log.length) {
        ...
        initDataTable(
            ...
            "undefined",
            "undefined",
            ActivityLogServerParams, [1, "desc"]
        );
    }
    ...
    if (table_invoices.length > 0 || table_estimates.length > 0) {
        ...
        $.each(Invoices_Estimates_Filter, function() {
            Invoices_Estimates_ServerParams[$(this).attr("name")] =
                '[name="' + $(this).attr("name") + '"]';
        });
        ...
        if (table_invoices.length) {
            ...
            initDataTable(
                ...
                "undefined",
                "undefined",
                Invoices_Estimates_ServerParams, !$("body").hasClass("recurring") ? [
                    [3, "desc"],
                    [0, "desc"],
        ...
        if (table_estimates.length) {
            ...
            initDataTable(
                ...
                "undefined",
                "undefined",
                Invoices_Estimates_ServerParams, [
                    [3, "desc"],
                    [0, "desc"],
                ]
            );
    ...
    if (table_tasks.length) {
        ...
        _table_api = initDataTable(
            ...
            tasksTableNotSortable,
            TasksServerParams, [table_tasks.find("th.duedate").index(), "asc"]
        );
            ...
            appCreateDropzoneOptions({
                ...
                success: function(files, response) {
                    ...
                    if (response.success === true || response.success == "true") {
                        ...
                        data +=
                            '<div class="pull-left"><i class="attachment-icon-preview fa fa-file-o"></i></div>';
                        data +=
                            '<a href="' +
                            site_url +
                            dl_url +
    ...
    if (billable_tasks_area.length > 0) {
        ...
        $.each(tasks, function(i, obj) {
            ...
            if (obj.started_timers === true) {
                option_data +=
                    'disabled class="text-danger important" data-subtext="' +
...
function init_rel_tasks_table(rel_id, rel_type, selector) {
    ...
    initDataTable(
        ...
        tasksRelationTableNotSortable,
        TasksServerParams, [$selector.find("th.duedate").index(), "asc"]
    );
}
...
function initDataTable(
    ...
    if (recordperpage != undefined) {
        app.options.tables_pagination_limit = recordperpage;
    }

    //console.log("RecordPerPage",recordperpage);

    if (columnattributes == undefined) {
        columnattributes = [];
    }
    //  console.log(columnDefs);
    //--  by Awnish
    console.log("PAGINATION_INFO", typeof PAGINATION_INFO);
    if (typeof PAGINATION_INFO != "undefined") {
        app.lang.datatables.info = PAGINATION_INFO;
    }
    // End
    var dtSettings = {
        ...
        responsive: true,
        autoWidth: false,
        dom: "<'row'><'row'<'col-md-7'lB><'col-md-5'f>>rt<'row table-footer'<'col-md-7'i><'col-md-5'<'#colvis'><'.dt-page-jump'>p>>",
        ...
        fnDrawCallback: function(oSettings) {
            ...
            if (oSettings.aoData.length === 0) {
                ...
                $(".buttons-collection").prop("disabled", true); //shikha
            } else {
        ...
        initComplete: function(settings, json) {
            ...
            if (
                t.hasClass("scroll-responsive") ||
                app.options.scroll_responsive_tables == 1
            ) {
    ...
    if (
        table.hasClass("scroll-responsive") ||
        app.options.scroll_responsive_tables == 1
    ) {
...
function init_table_tickets(manual) {
    ...
    _table_api = initDataTable(
        ...
        ticketsTableNotSortable,
        TicketServerParams, [tickets_date_created_index, "desc"]
    );
...
function timer_action(e, task_id, timer_id, adminStop) {
    ...
    if (timer_id !== "" && task_id == "0") {
        ...
        popupData.content +=
            '<div class="form-group"><select id="timer_add_task_id" data-empty-title="' +
...
function validate_invoice_form(selector) {
    ...
    appValidateForm($(selector), {
        clientid: {
            required: {
                depends: function() {
                    var customerRemoved = $("select#clientid").hasClass(
                        "customer-removed"
                    );
...
function set_notification_read_inline(id) {
    requestGet("misc/set_notification_read_inline/" + id).done(function() {
        ...
        notification
            .find(".notification-box,.notification-box-all")
            .removeClass("unread");
            ...
            if (nIds.length > 0) {
                $.each(nIds, function(i, notId) {
                    ...
                    $.notify("", {
                            ...
                            tag: notId,
                            closeTime: app.options.dismiss_desktop_not_after != "0" ?
                                app.options.dismiss_desktop_not_after * 1000 : null,
                        })
                        .close(function() {
        ...
        var options = {
            ajax: {
                url: typeof url == "undefined" ?
                    admin_url + "misc/get_relation_data" : url,
                data: function() {
                    var data = {};
                    data.type = type;
                    data.rel_id = "";
